...
// 5 x 28 = 120 bits for input_data
//wire [139:0] input_data; // 120-bit input data for each timestep

   

wire [27:0] weights;     // 28-bit weights read from memory
wire [27:0] output_data; // 28-bit output data from the LSTM controller
//wire rd_input;
...
wire locked,busy;
wire clear_all;
wire [27:0] final_output; // Store FC layer output for next timestep
...
fc_pe #(
    ...
    .ht_in(ht_data),
    .weight_in(weights),
    .bias_in(28'h001D7F7), // Example bias value (28 bits)
    .addr(addr_fc),
    .done(fc_done),
    .fc_out(final_output)
);
...
//assign SOC_MATCH = (28'h01f3f3 == SOC_out);
endmodule