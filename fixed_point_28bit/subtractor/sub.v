module subtractor #(
    ...
    wire [WIDTH-2:0] mag_a = a[WIDTH-2:0];  // 27 bits (7 int + 20 frac)
    wire [WIDTH-2:0] mag_b = b[WIDTH-2:0];  // 27 bits
    ...
    reg [WIDTH:0] temp_mag;      // One extra bit for overflow on magnitude (29 bits)
    reg temp_sign;
    ...
    // Maximum magnitudes for 1+7+20 format
    // Maximum positive magnitude: 0 1111111 11111111111111111111 = 127.99999904632568359375
    // Maximum negative magnitude: 0 1111111 11111111111111111111 = 127.99999904632568359375 (but with sign=1)
    localparam [WIDTH-2:0] MAX_MAG = 27'h7FFFFFF;  // Maximum magnitude (same for +/-)
